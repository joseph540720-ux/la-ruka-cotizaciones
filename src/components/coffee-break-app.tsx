...
import { formatCLP, formatDate, hoyLocal, lineSubtotal, needsWeeklyFollowUp, nextQuoteNumber, normalizeInvoiceInput, quoteTotals, type BusinessSettings, type Customer, type Product, type Quote, type QuoteDeliveryStatus, type QuoteItem, type QuoteStatus } from "@/lib/quote";
...
function QuoteWizard({ business, products, customers, quotes, initialQuote, mode, onAddCustomer, onCancel, onSave, onPrepareRecipient, onDone }: { business: BusinessSettings; products: Product[]; customers: Customer[]; quotes: Quote[]; initialQuote: Quote | null; mode: QuoteWizardMode; onAddCustomer: (customer: Customer) => void; onCancel: () => void; onSave: (quote: Quote) => Promise<void>; onPrepareRecipient: (target: EmailTarget, email: string, customerId: string) => Promise<string>; onDone: (quote: Quote) => void }) {
  ...
  return <section className="wizard">
    ...
    {step === 2 && <div className="quote-layout"><div className="wizard-card products-step"><div className="wizard-title"><span className="number">2</span><div><h2>Agrega productos</h2><p>Busca y presiona Agregar.</p></div></div><Search value={search} onChange={setSearch} placeholder="Buscar café, tapaditos, packs..."/><div className="catalog-list">{visibleProducts.map((product) => {
    ...
    })}</div></div><QuoteSummary items={items} totals={totals} updateItem={updateItem} removeItem={(id) => setItems(items.filter((item) => item.productId !== id))}/><div className="wizard-footer span-all"><button className="secondary" onClick={() => setStep(1)}>Volver</button><button className="primary" disabled={items.length === 0} onClick={() => setStep(3)}>Revisar cotización <Icon name="arrow"/></button></div></div>}
