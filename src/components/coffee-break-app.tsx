...
function QuoteEmailActions({ business, customer, busyTarget, onSend }: { business: BusinessSettings; customer: Customer; busyTarget: EmailTarget | null; onSend: (target: EmailTarget, recipient: string) => void }) {
  ...
  return <div className="email-options">
    <div className={`email-option ${recommended === "owner" ? "recommended" : ""}`}>
      ...
      <label>Mi correo<input aria-label="Destinatario de mi correo" type="email" value={ownerRecipient} onChange={(event) => setOwnerRecipient(event.target.value)}/></label>
...
function QuoteMobileCard({ quote }: { quote: Quote }) {
  ...
  const content = <>
    ...
    <span className="quote-mobile-meta"><span><small>Fecha</small>{formatDate(quote.date)}</span><span><small>Total</small><strong>{formatCLP(total)}</strong></span></span>
...
function QuoteTable({ quotes }: { quotes: Quote[] }) {
  return <><div className="table-wrap quote-table-desktop"><table><thead><tr><th>Número</th><th>Cliente</th><th>Fecha</th><th>Total</th><th>Envío</th><th>Respuesta</th><th>Facturación</th><th aria-label="Acciones"/></tr></thead><tbody>{quotes.map((quote) => <tr key={quote.id}><td><strong>{quote.number}</strong></td><td>{quote.customer.name}</td><td>{formatDate(quote.date)}</td><td><strong>{formatCLP(quoteTotals(quote.items).total)}</strong></td><td><DeliveryBadge status={quote.deliveryStatus}/></td><td><Badge status={quote.status}/></td><td>{quote.invoicedAmount ? <span className="billing-badge">Facturada · {formatCLP(quote.invoicedAmount)}</span> : <span className="billing-pending">Pendiente</span>}</td><td><Link className="icon-button" href={quotePath(quote.id)} aria-label={`Abrir ${quote.number}`}><Icon name="chevron"/></Link></td></tr>)}</tbody></table></div><div className="quote-mobile-list">{quotes.map((quote) => <QuoteMobileCard quote={quote} key={quote.id}/>)}</div></>;
}
...
function Quotes({ business, quotes, selectedQuoteId, onNew, onBack, onDuplicate, onEdit, onUpdate, onSave, onPrepareRecipient, onDelete }: { business: BusinessSettings; quotes: Quote[]; selectedQuoteId: string | null; onNew: () => void; onBack: () => void; onDuplicate: (quote: Quote) => void; onEdit: (quote: Quote) => void; onUpdate: (quote: Quote) => void; onSave: (quote: Quote) => Promise<void>; onPrepareRecipient: (target: EmailTarget, email: string, customerId: string) => Promise<string>; onDelete: (quote: Quote) => void }) {
  ...
  const [detailBusyTarget, setDetailBusyTarget] = useState<EmailTarget | null>(null); const [detailMessage, setDetailMessage] = useState(""); const [detailError, setDetailError] = useState(""); const [deliverySaving, setDeliverySaving] = useState(false);
  ...
  if (selected) {
    ...
    const saveInvoice = (data: FormData) => { const updated = withQuoteStatus({ ...selected, invoiceNumber: String(data.get("invoiceNumber") || ""), invoicedAt: String(data.get("invoicedAt") || ""), invoicedAmount: Number(data.get("invoicedAmount") || 0) }, "Aceptada"); onUpdate(updated); setBilling(false); };
...
function Products({ products, setProducts }: { products: Product[]; setProducts: (products: Product[]) => void }) {
  ...
  const saveProduct = (data: FormData) => {
    ...
    const product: Product = { id: editing?.id || crypto.randomUUID(), name, category, unit: String(data.get("unit") || "unidad").trim() || "unidad", price: Number(data.get("price")), cost: Number(data.get("cost")) || undefined, active: editing?.active ?? true };
  ...
  return <section className="panel"><div className="section-head responsive"><div><h2>Catálogo de productos</h2><p>{products.filter((p) => p.active).length} productos disponibles</p></div><button className="primary" onClick={() => { setEditing(null); setAdding(!adding); }}><Icon name="plus"/> Nuevo producto</button></div>
    {adding && <form className="inline-form" key={editing?.id || "new"} action={saveProduct}><label>Nombre<input name="name" required placeholder="Ej: Mini sándwich" defaultValue={editing?.name}/></label><label>Categoría<input name="category" list="product-categories" required placeholder="Elige o escribe una nueva" defaultValue={editing?.category}/><datalist id="product-categories">{categories.map((category) => <option value={category} key={category}/>)}</datalist></label><label>Unidad<input name="unit" defaultValue={editing?.unit || "unidad"}/></label><label>Precio neto<input name="price" type="number" min="0" required defaultValue={editing?.price}/></label><label>Costo<input name="cost" type="number" min="0" defaultValue={editing?.cost}/></label><button className="primary" type="submit">{editing ? "Actualizar" : "Guardar"}</button></form>}
